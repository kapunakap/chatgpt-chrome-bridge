...
WORK_ROOT="$REPO_ROOT/.tmp/browserjack-live-experiment"
...
printf '== disposable BrowserJack live experiment ==\n'
...
python3 - "$SRC_ROOT/src/discovery/app.ts" "$SRC_ROOT/src/discovery/native-host.ts" <<'PY'
...
printf 'remaining_checks=OpenAI TeamIdentifier + app/cache browser-client byte identity + real live runtime handshake\n\n'
...
printf '\nexperiment_exit=%s\n' "$STATUS"
if [[ "$STATUS" -eq 0 ]]; then
  echo 'EXPERIMENT_BROWSERJACK_LIVE_OK=1'
else
  echo 'EXPERIMENT_BROWSERJACK_LIVE_OK=0'
fi
